...
class DisassemblyListingRenderer:
    id = "disassembly"
    label = "Disassembly"
    ...
    def render(self, bv: "BinaryView", func: "Function", params: ListingParams) -> Listing:
        ...
        if not func:
            return Listing(text="error: invalid function for disassembly listing.", line_count=0, truncated=False)

        address_width = params.address_width
        if address_width <= 0:
            address_width = func.arch.address_size * 2 if func.arch else 8

        raw_lines = get_function_code_lines(func, CodeDisplayType.DISASSEMBLY, logger)
        ...
        text = "\n".join(truncated_lines) if truncated_lines else "-- no disassembly lines --"
